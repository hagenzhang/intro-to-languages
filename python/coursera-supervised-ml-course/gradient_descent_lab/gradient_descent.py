...
def compute_cost(x, y, w, b):
    ...
    Returns
      total_cost (int): cost of the given model 
    """
...
# initialize parameters (convention to start all params at 0)
w_init = 0
b_init = 0
# some gradient descent settings
iterations = 10000
tmp_alpha = 1.0e-2  # learning rate of 0.01
# run gradient descent
w_final, b_final, J_hist, p_hist = gradient_descent(x_train ,y_train, w_init, b_init, tmp_alpha, 
                                                    iterations, compute_cost, compute_gradient)
print(f"\n(w,b) found by gradient descent: ({w_final:8.4f},{b_final:8.4f})\n")
print("Final Cost: ", compute_cost(x_train, y_train, w_final, b_final))

"""
Some characteristics of the gradient descent process:
 - The cost starts large and rapidly declines as described in the slide from the lecture.
 - The partial derivatives, dj_dw, and dj_db also get smaller, rapidly at first and then more slowly.
 - as the process approaches the global minimum, progress is slower due to the smaller value of the
...
# In the resulting plots, note the scale of cost on the axes and the iteration step.#
...
# Now that we have the optimal values for the parameters 𝑤 and 𝑏, 
# we can use the model to predict housing values based on our learned parameters. 
# As expected, the predicted values are nearly the same as the training values for
# the same housing. 
